...
def call_api_with_para(user_id, api_name, param_dict):
    domain_list = get_uc_internal_domain_list()
    form_data_dict = {"user_id": user_id, "api_name": api_name}
    if param_dict:
        form_data_dict["parameters"] = json.dumps(param_dict, ensure_ascii=False)
    resp_data = try_send_http_request(domain_list=domain_list, path=settings.API_USER_CENTER_CALL_API, method="POST", form_data_dict=form_data_dict)
    return resp_data
