...
def get_day_list(startdate, enddate):
    ...
    if startdate > enddate:
        raise BusinessException(TIME_ERROR_START_END)
...
def get_cycletime_from_date_type(date_type, startdate=None, enddate=None, offset=0):
    ...
    if date_type:
        ...
        if date_type == DATE_TYPE_STUDYYEAR:
            ...
            raise BusinessException(TIME_ERROR)
    else:
...
def compose_hour_minute(hour, minutes):
    try:
        if int(hour) > 24 or int(hour) < 0 or int(minutes) > 60 or int(minutes) < 0:
            raise Exception()
...
def day_between(begin_dt, end_dt):
    date_list = []
    # begin_date = datetime.datetime.strptime(begin_date, "%Y-%m-%d")
    # end_date = datetime.datetime.strptime(end_date,"%Y-%m-%d")
    while begin_dt <= end_dt:
        # date_str = end_dt.strftime('%Y-%m-%d')
        date_list.append(begin_dt)
        begin_dt += datetime.timedelta(days=1)
    return date_list


def datetime_offset_by_month(datetime1, n=1, fix_last_day=False):
    ...
    if (not fix_last_day) and (datetime1.month != (datetime1 + one_day).month):
        return datetime2
