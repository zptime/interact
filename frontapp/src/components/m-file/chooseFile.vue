<template>
    <span>
      ...
      <m-alert ref="tipTxtAlert"></m-alert>
    </span>
</template>
...
<script>
  ...
  import mAlert from '../../components/m-alert/index.vue'
  ...
  import Vue from 'vue'
    ...
    export default {
        ...
        data(){
          return {
            downloadArr:[],//文件下载的数组
            fileInPhone:[],//从手机获取的所有文件的数组，除非换了界面，不然不会清空，用于判断文件存不存在手机中
        ...
        methods:{
          ...
          downloadFileResult(data){
            ...
            let  fileName;
            for(let k =0,kLen=this.downloadArr.length;k<kLen;k++){
              let item = this.downloadArr[k];
              if (item.file_url == file_url){
                fileName = item.file_name;
              }
            }
            this.fileInPhone.push(fileName);
            window.mFilePhoneFile = this.fileInPhone;
            this.getPhoneFileResult();
          },
          getPhoneFileResult(data){
            if(data && data.length > 0){
              this.fileInPhone = data;
              window.mFilePhoneFile = this.fileInPhone;
            }
            for (let i =0,len = this.fileData.length;i<len;i++){
              let fileObj = this.fileData[i];
              for (let j=0,jlen=this.fileInPhone.length;j<jlen;j++){
                let fileNameStr = this.fileInPhone[j];
                if (fileObj.file_name == fileNameStr){
                  fileObj.downloadState = -1;//查看
                  Vue.set(this.fileData,i,fileObj);
                }
              }
            }
          },
          //微信中文件点击的时间回调
          weiXinFileClick(data){
            let obj = this.$refs.tipTxtAlert;
            obj.needPasedTxt = data.file_url;
            obj.show = true;
          },
          //文件预览
          ...
          clickRightButtotn(item,index){
            //点击下载
             if(item.downloadState == 0){
               ...
               Vue.set(this.fileData,index,item);
               this.downloadArr.push(item);
               return;
             }
            //点击查看
          ...
          setUp(){
            ...
            if (!this.showDelete && this.fileData.length > 0 && !window.mFilePhoneFile){
              this.nativeFile.getPhoneFile(this.fileData);
            }
            if (window.mFilePhoneFile){
              this.fileInPhone = window.mFilePhoneFile;
              this.getPhoneFileResult();
            }
          },
          //文件大小
      ...
      * */
        ...
        destroyed(){
          ...
          //处理时机不好判断，先这样处理，华校中不用清空mFilePhoneFile
          if (window.mFilePhoneFile){
            // window.mFilePhoneFile = undefined;
          }
        }
    }
