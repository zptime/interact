...
function weixinFile (target, opt){
  ...
  this.init = function (obj) {
    ...
    this.filePreView = function (obj) {
      this.weiXinFileClick(obj);
    },
    this.weiXinFileClick = function (data) {
      var fileObj = {}
      if (data){
        try {
...
let asyncUpload = async (fileMgrSelf)=> {
  if (!fileMgrSelf.target.$refs.uFile.files[0]) {
    return;
  }

  //alert(fileMgrSelf.target.$refs.uFile.value);
  showTextLoading(fileMgrSelf.target,'上传中');
  let res = await uploadFile(fileMgrSelf.target.$refs.uFile.files[0]);
  fileMgrSelf.target.$refs.uFile.value = '';
  if (res.c == 0 ) {
