<template>
  <div class="member">
    <div class="main-top" v-if="showSearch || showChoose">
    ...
    <div :class="{'member-list-common':innerScroll,'item':true}" :style="setMemberStyle" v-for="(member,parentIndex) in members" v-if="member.number>0 && filterData(member.data).length>0">
        ...
        <div v-for="(itemData,index) in filterData(member.data)" :class="member.show?'show':'hide'">
          <div class="item-detail clb" :class="{'item-border-none': index==member.data.length-1}">
    ...
    <div class="main-bottom btnOpt" v-if="showOptBtn">
...
//          'width':3*count+ 'rem'
      ...
      toggleCheckAll(pIndex){
        ...
        for(let i=0;i<this.members[pIndex].data.length;i++){
          ...
          //搜索条件过滤
          if(obj.username.search(this.keyword)!=-1){
