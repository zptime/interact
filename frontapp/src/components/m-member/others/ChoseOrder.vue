  ...
  <div class="member">
    ...
    <div class="contentBox" v-cloak>
      ...
      <div class="list-common">
        <div class="item" v-for="(item,pIndex) in orderOptData">
          <div class="item-title item-letter" v-if="orderFlag">
            {{ item.letter }}
          </div>
          <div class="item-title clb" v-else>
          ...
          <div v-for="(itemData,index) in item.list">
            <div class="item-detail clb" @click="toggleCheckOne(index,pIndex)" v-if="itemData.username.search(keyword)!=-1" :class="{'item-border-none': index==item.list.length-1}">
    ...
    methods:{
      ...
      toggleCheckAll(){
        if(this.checkVal=='全选'){
          ...
          for(let i=0;i<this.orderOptData.length;i++){
            for(let j=0;j<this.orderOptData[i].list.length;j++){
              this.orderOptData[i].list[j].is_checked = true;
            }
          }
        }else{
          this.chooseNum = 0;
          for(let i=0;i<this.orderOptData.length;i++){
            for(let j=0;j<this.orderOptData[i].list.length;j++){
              this.orderOptData[i].list[j].is_checked = false;
            }
          }
        }
      },
      ...
      //选择、取消选择事件
      toggleCheckOne(index,pIndex){
        let tmp = this.orderOptData[pIndex].list[index].is_checked;
        if(tmp){
          this.chooseNum-=1;
        }else{
          this.chooseNum+=1;
        }
        this.orderOptData[pIndex].list[index].is_checked = !tmp;
      },
      ...
      async optClick(opt){
        if(opt=='cancel'){//取消
          ...
          for(let i=0;i<this.orderOptData.length;i++){
            for(let j=0;j<this.orderOptData[i].list.length;j++){
              this.bulkOptData.push(this.orderOptData[i].list[j]);
            }
          }
          let reData = {
