<template>
  <div>
    <div class="contentBox" v-if="showPage" v-cloak>
      ...
      <popup v-model="showPopup" width="100%" height="100%" position="right" :show-mask=false>
        <chose-order :head-title="headTitle" :bulk-data="bulkData" :data-index="dataIndex" :orderFlag="true"
...
//          this.showList = true;
      ...
      //获取班级学生列表
      async getClassStudents(index){
        let res = await commonClassStudentList(this.classList[index].id);
        if(res.c==0){
          this.classList[index].data = res.d;
          for(let i=0;i<this.classList[index].data.length;i++){
            this.classList[index].data[i].is_checked = false;
          }
        }else{
          this.$vux.toast.show({
            type: 'text',
      ...
      //获取群组学生列表
      async getGroupStudents(index){
        let res = await contactGroupUserList(this.groupList[index].id);
        if(res.c==0){
          this.groupList[index].data = [];
          for(let i=0;i<res.d.length;i++){
            if(res.d[i].user_type_id==1){
              res.d[i].is_checked=false;
              this.groupList[index].data.push(res.d[i]);
            }
          }
          this.groupList[index].total = this.groupList[index].data.length;
        }else{
          this.$vux.toast.show({
            type: 'text',
            text: res.m
