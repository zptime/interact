<template>
  <div>
    ...
    <div class="contentBox"  style="height: 100%;"   v-cloak >
                ...
                <Scroll :on-refresh="onRefresh" :on-infinite="onInfinite" >
                     <div  style="overflow: scroll;padding: 0 0.75rem;background: #FFFFFF;padding-bottom: 1rem;" >
                                    <div v-for="item in listdata"  @click="go_detail(item.notify_id,index)" class="item"   :class="{ p2rem : showbottom }" >
                                        ...
                                        <div class="img-box" :class=" { l2rem : showbottom }"   ><img v-if="item.sender_avatar !='' "   :src="item.sender_avatar" alt=""><img v-if="item.sender_avatar =='' "   src="../../../images/icon-default-avatar.png" alt=""><div class="item-read" v-if="item.is_read==0"></div></div>
                                        <div class="item-content">
                                            <div class="item-top"><span class="item-type">{{ item.type_desc }} - </span><span   class="item-name">{{ item.sender_username }}</span><span class="item-time">{{ item.create_time.substring(10) }}</span></div>
                                    ...
                                    <div v-for="(item,index) in downdata"  @click="go_detail(item.notify_id)" class="item" :class="{ p2rem : showbottom }" >
                                        <div v-if="showbottom" class="notselect" :class="{ hasselect : item.isselect }"  @click.stop=" item.isselect = !item.isselect; " ></div>
                                        <div class="img-box" :class=" { l2rem : showbottom }"   ><img  :src="item.sender_avatar" alt=""><div class="item-read" v-if="item.is_read==0"></div></div>
                                        <div class="item-content">
                                            <div class="item-top"><span class="item-type">{{ item.type_desc }} - </span><span   class="item-name">{{ item.sender_username }}</span><span class="item-time">{{ item.create_time.substring(10) }}</span></div>
                                            <div class="item-txt">{{ item.content }}</div>
                                        </div>
            ...
            <div class="hover-publish" v-if="index==0&&!showdialog&&!showbottom"  @click="showbottom= true ;"  ></div>

            <div class="fix-bottom" v-if="showbottom">
            <div @click="chooseAll()">{{ chooseAllTxt }}</div>
            <div class="b-left"  @click="set_hasread()">标为已读</div>
            <div class="b-left"  @click="deleteItem(0)">删除所选</div>
            <div class="b-left"  @click="deleteItem(1)">清空</div>
            <div class="b-left"  @click="showbottom=false">返回</div>
        </div>
    </div>
  </div>
</template>
<style scoped>
    ...
    .item .img-box{position: absolute;left: 0rem;top: 0.5rem;width: 2.25rem;height: 2.25rem;border-radius: 50%;border: solid 1px #eeeeee;}
    ...
    .d-inner{width: 6rem;height: auto;position: fixed;right: 1.125rem;bottom: 3.375rem;}
    ...
    .fix-bottom{border-top: solid 1px #d7d7d7;height: 2.2rem;width: 100%;box-sizing: border-box;position: fixed;background: #fff;left: 0;bottom: 0;text-align: center;line-height: 2.2rem;color: #FFFFFF;font-size: 0.8rem;}
    .fix-bottom div{float: left;width: 20%;box-sizing: border-box;color: #4685ff;}
...
<script type="text/ecmascript-6">
  ...
  import { getTeacherNoticeList , deleteNotice ,getReceiverList,setReaded , setUnread ,deleteReceiverNotice }  from '../../../service/getData.js'
  ...
  export default {
    ...
    methods:{
        ...
        async deleteItem(type){
            ...
            for(let i in this.downtdata){
                if(this.downdata[i].isselect){
                    ...
                    newdownarr.push(this.downdata[i]);
                }
            }
            ...
            let res = await deleteReceiverNotice(notify_ids,type);
        ...
        async set_hasread(){//标记已读
            let ids = [];
            for(let i in this.listdata){
                if(this.listdata[i].isselect){
                    ids.push(this.listdata[i].notify_id)
                    this.listdata[i].is_read=1;
                }
            };
            for(let i in this.downdata){
                if(this.downdata[i].isselect){
                    ids.push(this.downdata[i].notify_id)
                    this.downdata[i].is_read=1;
                }
            };
            if(ids.length==0)return;
            this.$vux.loading.show();
            let res = await setReaded(ids);
            this.$vux.loading.hide();
        },
        async set_notread(){//标记未读
            let ids = [];
            for(let i in this.listdata){
                if(this.listdata[i].isselect){
                    ids.push(this.listdata[i].notify_id)
                    this.listdata[i].is_read=0;
                }
            };
            for(let i in this.downdata){
                if(this.downdata[i].isselect){
                    ids.push(this.downdata[i].notify_id)
                    this.downdata[i].is_read=0;
                }
            };
            if(ids.length==0)return;
            this.$vux.loading.show();
            let res = await setUnread(ids);
            this.$vux.loading.hide();
        },
        go_send_notice(type){
        ...
        go_detail(notify_id){
            ...
            this.$router.push({ path: "/m/notice/parent/noticeDetail" , query: { notify_id: notify_id } });
        },
        async getList(rows){
            let res = await getReceiverList(rows,this.search.search_txt,'');
            let arr = [];
            if(res.c==0){
                arr = res.d.data_list;
                for(let i in arr){
        ...
        async onRefresh(done){
            let res = await getReceiverList(10,this.search.search_txt,'');
        ...
        async onInfinite(done){
          ...
          let res = await getReceiverList(10,this.search.search_txt,last_id);
          let arr = res.d.data_list;
          for(let i in arr){
              arr[i].isselect = false;
          }
