<template>
  <div v-cloak>
    <head-top :head="head">
      ...
      <div style="padding: 2.1rem 0rem 0rem 0rem;background: #FFFFFF;">
...
<script type="text/ecmascript-6">
  ...
  import {initializeEnv} from '../../../components/framework/serviceMgr.js'
  import {closeWindows} from  '../../../components/framework/activityMgr.js'
  export default {
    data () {
      return {
        head:{
    ...
    methods:{
       goBack() {
         if (this.$route.query.fromGo == "list") {
           closeWindows(this);
         } else {
           this.$router.back();
         }
        ...
        async initDetail(notify_id){
            ...
            if(res.c==0){
              ...
              //      o.file_name = res.d.files[i].file_name;
               //     res.d.fileso.file_id = res.d.files[i].file_id;
                //    o.file_url = res.d.files[i].file_url;
                 //   o.file_size = res.d.files[i].file_size;
                }
                this.voiceArr = [];
