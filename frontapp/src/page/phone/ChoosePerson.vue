...
<script type="es6">
  ...
  import { getClassList,contactGroupCreate,contactGroupInviteAgain,contactGroupUserInvite,contactClassUserList } from '../../service/getData'
  export default {
    data(){
      return {
        head:{
    ...
    methods:{
      initData(){
        ...
        if(this.$route.query.groupId){
          ...
          this.getInviteData();
        }else if(this.$route.query.gName){
          //传入群组名称 -- 创建群组添加成员
        ...
        for(let i=0;i<this.classItems.length;i++){
          ...
          if(res.c==0){
            ...
            for(let i=0;i<res.d.length;i++){
              if(res.d[i].user_type_id=='1'){//只显示学生
